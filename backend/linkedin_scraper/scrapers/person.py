...
from ..models import Person, Experience, Education, Accomplishment, Interest, Contact, Skill
...
class PersonScraper(BaseScraper):
    ...
    async def _get_skills(self, base_url: str) -> list[Skill]:
        """Extract skills from the LinkedIn Skills details page.

        Falls back to the main-profile Skills section when the details page
        is not available. Each skill captures its name and endorsement count.
        """
        skills: list[Skill] = []
        ...
        try:
            ...
            if not items:
                items = await self.page.locator("main ul > li, main ol > li").all()
            ...
            if not skills:
                skills = await self._get_skills_from_main_page()
        except Exception as e:
    ...
    async def _get_skills_from_main_page(self) -> list[Skill]:
        """Best-effort fallback: read skills from the main profile rail."""
        skills: list[Skill] = []
        try:
            skill_heading = self.page.locator('h2:has-text("Skills")').first
            if await skill_heading.count() == 0:
                return skills
            section = skill_heading.locator('xpath=ancestor::*[.//ul or .//ol][1]')
            if await section.count() == 0:
                section = skill_heading.locator('xpath=ancestor::*[4]')
            if await section.count() == 0:
                return skills
            items = await section.locator('ul > li, ol > li').all()
            for item in items:
                texts = await self._extract_unique_texts_from_element(item)
                if texts:
                    name = texts[0]
                    if name and len(name) < 200:
                        skills.append(Skill(name=name.strip()))
        except Exception as e:
            logger.debug(f"Error getting skills from main page: {e}")
        return skills

    async def _parse_skill_item(self, item) -> Optional[Skill]:
        """Parse a single skill list item from the Skills details page."""
        try:
            entity = item.locator(
                'div[data-view-name="profile-component-entity"]'
            ).first
            ...
            for i, span in enumerate(spans[:4]):
                ...
                if i == 0:
                    name = text
                elif "endorsement" in text.lower():
                    digits = "".join(ch for ch in text if ch.isdigit())
                    endorsements = int(digits) if digits else None

            if not name:
                unique = await self._extract_unique_texts_from_element(item)
                if unique:
                    name = unique[0]
            ...
            return Skill(name=name.strip(), endorsements=endorsements)
        except Exception as e:
            logger.debug(f"Error parsing skill item: {e}")
            return None
